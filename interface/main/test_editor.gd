...
func load_graph_state(graph_id: String, expected_input_count: int = 1):
	...
	if data.get("nodes", {}).is_empty():
		if graph_id == "main":
			var test_cat = "Testing (部署推理)"
			var sys_cat_ref = "System (系统节点)"
			
			if GlobalData.node_library.has(test_cat):
				var n_cfg = create_node(test_cat, "Inference Config", Vector2(50, 50))
				n_cfg.name = "Inference_Config"
				
				var n_in = create_node(sys_cat_ref, "Data Input", Vector2(50, 250))
				n_in.name = "input0"; n_in.title = "Data Input"
				
				var n_out = create_node(sys_cat_ref, "Data Output", Vector2(800, 250))
				n_out.name = "output0"; n_out.title = "Data Output"
				
				var n_comment = create_node(sys_cat_ref, "Comment (注释)", Vector2(300, 50))
				n_comment.name = "Workflow_Comment"
				if n_comment.config.has("params"):
					n_comment.config.params[0]["value"] = "【推理工作流】\n1. 在上方 📦导入模型\n2. 将模型置于此处\n3. 连线: Input -> 模型 -> Output"
				
				await get_tree().process_frame
				_request_shape_inference()
			else:
				print("警告：未找到 Testing (部署推理) 节点库！请检查 Python 后端。")
...
func _get_selected_nodes() -> Array:
	var selected = []
	for n in graph_edit.get_children():
		if n is GraphNode and n.selected: selected.append(n)
	return selected
...
func _refresh_breadcrumbs():
	...
	for i in range(path.size()):
		var btn = Button.new()
		btn.text = "Test" if str(path[i]) == "main" else str(path[i])
