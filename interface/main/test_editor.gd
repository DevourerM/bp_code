...
func save_current_graph_state():
	...
	GlobalData.project_graphs["test"] = data
...
func load_graph_state():
	...
	var data = GlobalData.project_graphs.get("test", {})
	if data.get("nodes", {}).is_empty():
		if GlobalData.node_library.has("Testing (部署推理)"):
			var cfg = create_node("Testing (部署推理)", "Inference Config", Vector2(50, 50))
			cfg.name = "Config"
	else:
